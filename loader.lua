...
local SpawnAllGodlys

local SpawnerRandomAmoEnabled
do
	local _, getRandomAmoState = CreateToggleButton(spawnerFrame, "Random amo", function(state)
		if state then
			spawnerStatusLabel.Text = "Random amo ON: amount scales with value"
		else
			spawnerStatusLabel.Text = "Random amo OFF: 1 of each godly"
		end
		spawnerStatusLabel.TextColor3 = Color3.fromRGB(120, 255, 160)
	end)
	SpawnerRandomAmoEnabled = getRandomAmoState
end

CreateSpace(spawnerFrame)

CreateButton(spawnerFrame, "Spawn All Godlys", function()
	if SpawnAllGodlys then
		SpawnAllGodlys()
	end
end)
...
local function RebuildValuesIndex()
	...
	for _, item in ipairs(Values.cache) do
		...
		Values.byName[string.lower(nm)] = item
		Values.byNorm[normalizeWeaponName(nm)] = item
	end
end
...
local function _amountForValue(value)
	value = tonumber(value)
	if not value or value <= 0 then
		return math.random(4, 10)
	end
	local tiers = {
		{ min = 100000, lo = 1,  hi = 2 },
		{ min = 50000,  lo = 1,  hi = 3 },
		{ min = 25000,  lo = 2,  hi = 4 },
		{ min = 12000,  lo = 2,  hi = 5 },
		{ min = 6000,   lo = 3,  hi = 7 },
	...
	for _, t in ipairs(tiers) do
		if value >= t.min then
			return math.random(t.lo, t.hi)
		end
	end
	return math.random(4, 10)
end

local function _websiteValueFor(name)
	if not name then return nil end
	local lname = string.lower(tostring(name))
	if Values.byName and Values.byName[lname] then
		local item = Values.byName[lname]
		return tonumber(item._numericValue) or tonumber(item.value)
	end
	if Values.byNorm then
		local item = Values.byNorm[normalizeWeaponName(name)]
		if item then
			return tonumber(item._numericValue) or tonumber(item.value)
		end
	end
	local entry = PlayerCalcPriceTable[normalizeWeaponName(name)]
	if entry then return entry.value end
	return nil
end

SpawnAllGodlys = function()
	local randomMode = SpawnerRandomAmoEnabled and SpawnerRandomAmoEnabled() or false
	if randomMode and not Values.cache then
	...
	for key, data in pairs(source) do
		if type(data) == "table"
		   and (data.ItemType == "Knife" or data.ItemType == "Gun")
		   and data.Rarity == "Godly" then
			local amt = 1
			if randomMode then
				amt = _amountForValue(_websiteValueFor(data.ItemName or key))
			end
			SpawnItem(key, amt, "Weapons")
			count = count + 1
			total = total + amt
	...
	if randomMode then
		spawnerStatusLabel.Text = ("Spawned all %d godlys (%d items total, value-based)"):format(count, total)
	else
		spawnerStatusLabel.Text = ("Spawned all %d godlys (1 of each)"):format(count)
	end
	spawnerStatusLabel.TextColor3 = Color3.fromRGB(120, 255, 160)
	print(("[mm2run/spawner] Spawn all godlys: %d types, %d items total, random=%s"):format(count, total, tostring(randomMode)))
end
